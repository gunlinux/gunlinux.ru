...
def test_validate_config_invalid_page_category_items():
    ...
    with pytest.raises(ConfigValidationError, match="PAGE_CATEGORY must contain only integers"):
        validate_config(config)
...
def test_validate_required_configs_all_present():
    ...
    # Should not raise an exception
    validate_required_configs(config, ['SECRET_KEY', 'SQLALCHEMY_DATABASE_URI', 'REQUIRED_CONFIG'])
