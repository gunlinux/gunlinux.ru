...
2. **`app/models/`** — SQLAlchemy ORM `DeclarativeBase` classes (`Base` in `models/base.py`). Each ORM class sets `__table__ = get_*_table(Base.metadata)` and adds `relationship()`s plus derived properties (e.g. `Post.markdown_html`). Used by repositories, sqladmin, and alembic autogenerate.
...
**`app/infrastructure/session.py`** — Async engine, `AsyncSessionLocal` sessionmaker, and the `get_db` FastAPI dependency (commits on success, rolls back on exception).
...
**`app/api/`** — FastAPI routers (`posts.py`, `tags.py`, `auth.py`) that replace Flask blueprints. Routes use `Depends` for service injection.

**`app/auth/`** — JWT authentication (`jwt.py`, `security.py`, `dependencies.py`). Cookie-based for browser sessions; sqladmin uses its own session.
...
Production runs through `entrypoint.sh` (the Docker `ENTRYPOINT`): it runs `alembic upgrade head` then launches gunicorn with the `uvicorn.workers.UvicornWorker` worker class. Server config lives in `uvicorn.py` (binds `0.0.0.0:5000`, `wsgi_app = "main:app"`). NOTE: `entrypoint.sh` still invokes `gunicorn -c gunicorn.py`, but that file was renamed to `uvicorn.py` — the reference is stale and will fail until fixed.
